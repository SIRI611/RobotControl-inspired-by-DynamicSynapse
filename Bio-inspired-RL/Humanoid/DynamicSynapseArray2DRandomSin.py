...
class DynamicSynapseArray:
    def __init__(self, NumberOfSynapses = [1,3], Period=None, tInPeriod=None, PeriodVar=None,\
                ...
                ModulatorAmount=0, InitAmp=0.4, t = 0, dt=1, NormalizedWeight=False, Amp2=0.2):
        ...
        self.dt = dt
        self.t = t
        self.PeriodCentre=np.ones(NumberOfSynapses).astype(float)*Period if Period is not None else 1000+100*(np.random.rand(*NumberOfSynapses)-0.5)
        self.Period = copy.deepcopy(self.PeriodCentre)
        self.tInPeriod=np.ones(NumberOfSynapses).astype(float)*tInPeriod  if tInPeriod is not None else np.random.rand(*NumberOfSynapses)*self.Period
        
        self.PeriodVar = np.ones(NumberOfSynapses).astype(float)*PeriodVar if PeriodVar is not None else np.ones(NumberOfSynapses).astype(float)*0.1
        self.Amp = np.ones(NumberOfSynapses).astype(float)*Amp if Amp is not None else np.ones(NumberOfSynapses).astype(float)*0.2

        self.WeightersCentre = np.ones(NumberOfSynapses)*WeightersCentre if WeightersCentre is not None else (np.random.rand(*NumberOfSynapses)-0.5)*Amp2
...
#        self.Period[self.ZeroCross] += (np.random.rand(*self.NumberOfSynapses)[self.ZeroCross]-0.5)*self.PeriodVar[self.ZeroCross]*self.Period[self.ZeroCross]+(self.PeriodCentre-self.Period)[self.ZeroCross]*0.03
    ...
    def Recording(self):
        ...
        for key in self.Trace:
            Temp = []
            exec("Temp.append(self.%s)" % (key))
            self.Trace[key].append(copy.deepcopy(Temp[0]))
...
#            Trace = self.Trace
        ...
        SynapseID=0
        print(np.array(self.Trace['Weighters'])[::DownSampleRate])
...
if __name__=="__main__":
    ...
    if InintialDS:
        ...
        SimulationTimeLenth = 100*60*10  
        Period = 20000
        dt = SimulationTimeInterval = 3
        ...
        ModulatorAmount[int(PeriodSteps/2):PeriodSteps]=1
        ModulatorAmount[PeriodSteps*2:PeriodSteps*2+int(PeriodSteps/2)]=1
        NumberOfNeuron=2
        NumberOfSynapses = 3
...
#%%
        FigureDict,ax = ADSRA.plot( path='~/Documents/DSA/SimulationResult/Plots/',DownSampleRate=1, savePlots=True, linewidth= 0.2, NameStr=TimOfRecording) #path=
#%%
