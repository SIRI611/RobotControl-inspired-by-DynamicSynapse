...
import argparse
import Humanoid
# from Humanoid.Adapter.RangeAdapter import RangeAdapter
# from Humanoid.Adapter.tracelogger import TraceLogger
...
if __name__ == "__main__":
    ...
    if new:
        ...
        NumberOfSynapses = np.array([64, (num_states-331)*2+(331)+1+num_actions])

        dt = 3*5
        N_episode = 10000
        T = 0
        # range_adapter = Adapter.RangerAdapter.RangeAdapter()
        ...
        #left arm
        # WeightersCentre[48:56, 0:21] = (np.random.rand()-0.5)*0.005
        WeightersCentre[48:56, 21:22] = (np.random.rand()-0.5)*0.1
        # WeightersCentre[48:56, 22:44] = (np.random.rand()-0.5)*0.005
        WeightersCentre[48:56, 44:45] = (np.random.rand()-0.5)*0.1
        # WeightersCentre[48:56, 45:66] = (np.random.rand()-0.5)*0.005
        WeightersCentre[48:56, 66:67] = (np.random.rand()-0.5)*0.1
        # WeightersCentre[48:56, 67:89] = (np.random.rand()-0.5)*0.005
        WeightersCentre[48:56, 89:90] = (np.random.rand()-0.5)*0.1
        # WeightersCentre[48:56, 425:438] = (np.random.rand()-0.5)*0.005
        WeightersCentre[48:56, 438:439] = (np.random.rand()-0.5)*0.1

        #right arm
        # WeightersCentre[56:64, 0:18] = (np.random.rand()-0.5)*0.005
        WeightersCentre[56:64, 18:19] = (np.random.rand()-0.5)*0.1
        # WeightersCentre[56:64, 19:41] = (np.random.rand()-0.5)*0.005
        WeightersCentre[56:64, 41:42] = (np.random.rand()-0.5)*0.1
        # WeightersCentre[56:64, 42:63] = (np.random.rand()-0.5)*0.005
        WeightersCentre[56:64, 63:64] = (np.random.rand()-0.5)*0.1
        # WeightersCentre[56:64, 64:86] = (np.random.rand()-0.5)*0.005
        WeightersCentre[56:64, 86:87] = (np.random.rand()-0.5)*0.1
        # WeightersCentre[56:64, 87:90] = (np.random.rand()-0.5)*0.005
        # WeightersCentre[56:64, 425:435] = (np.random.rand()-0.5)*0.005
        WeightersCentre[56:64, 435:436] = (np.random.rand()-0.5)*0.1
        ...
        NumberOfSynapses2 = np.array([16, 64+1])
        ...
        WeightersCentre2[14:16, 56:64] = (np.random.rand()-0.5)*0.12
        ...
        ADSA_l1 = DSA.DynamicSynapseArray(np.array([1,20]) , Period=20000, tInPeriod=None, PeriodVar=0.1,\
                 Amp=0.2, WeightersCentre = np.zeros(np.array([1,20])), WeightersCentreUpdateRate = 0.000012, WeightersOscilateDecay=0.0000003/100)
        ADSA_r1 = DSA.DynamicSynapseArray(np.array([1,20]) , Period=20000, tInPeriod=None, PeriodVar=0.1,\
                 Amp=0.2, WeightersCentre = np.zeros(np.array([1,20])), WeightersCentreUpdateRate = 0.000012, WeightersOscilateDecay=0.0000003/100)
        ADSA_l2 = DSA.DynamicSynapseArray(np.array([1,19]) , Period=20000, tInPeriod=None, PeriodVar=0.1,\
                 Amp=0.2, WeightersCentre = np.zeros(np.array([1,19])), WeightersCentreUpdateRate = 0.000012, WeightersOscilateDecay=0.0000003/100)
        ADSA_r2 = DSA.DynamicSynapseArray(np.array([1,19]) , Period=20000, tInPeriod=None, PeriodVar=0.1,\
                 Amp=0.2, WeightersCentre = np.zeros(np.array([1,19])), WeightersCentreUpdateRate = 0.000012, WeightersOscilateDecay=0.0000003/100)
        ADSA_l3 = DSA.DynamicSynapseArray(np.array([1,18]) , Period=20000, tInPeriod=None, PeriodVar=0.1,\
                 Amp=0.2, WeightersCentre = np.zeros(np.array([1,18])), WeightersCentreUpdateRate = 0.000012, WeightersOscilateDecay=0.0000003/100)
        ADSA_r3 = DSA.DynamicSynapseArray(np.array([1,18]) , Period=20000, tInPeriod=None, PeriodVar=0.1,\
                 Amp=0.2, WeightersCentre = np.zeros(np.array([1,18])), WeightersCentreUpdateRate = 0.000012, WeightersOscilateDecay=0.0000003/100)
        ADSA_leftarm = DSA.DynamicSynapseArray(np.array([1,18]) , Period=20000, tInPeriod=None, PeriodVar=0.1,\
                 Amp=0.2, WeightersCentre = np.zeros(np.array([1,18])), WeightersCentreUpdateRate = 0.000012, WeightersOscilateDecay=0.0000003/100)
        ADSA_rightarm = DSA.DynamicSynapseArray(np.array([1,18]) , Period=20000, tInPeriod=None, PeriodVar=0.1,\
                 Amp=0.2, WeightersCentre = np.zeros(np.array([1,18])), WeightersCentreUpdateRate = 0.000012, WeightersOscilateDecay=0.0000003/100)
        
        NumberOfSynapses3 = np.array([num_actions, 8+22+1])
        InitWeights = 0.8
        WeightersCentre3 = np.zeros(NumberOfSynapses3)
        # WeightersCentre3[0:3,13:16] = -InitWeights/3
        WeightersCentre3[0:3, :] = 0.02
        # WeightersCentre3[:, 53:54] = 0.02
        WeightersCentre3[3:6, 3:4] = InitWeights/2
        WeightersCentre3[3:6, 16:19] = -InitWeights/3
        # WeightersCentre3[3:6, 39:42] = -0.02

        WeightersCentre3[6:7, 5:6] = InitWeights/2
        WeightersCentre3[6:7, 19:20] = -InitWeights
        # WeightersCentre3[6:7, 42:43] = -0.02

        WeightersCentre3[7:10, 2:3] = InitWeights/2
        WeightersCentre3[7:10, 20:23] = -InitWeights/3
        # WeightersCentre3[7:10, 43:46] = -InitWeights

        WeightersCentre3[10:11, 4:5] = InitWeights/2
        WeightersCentre3[10:11, 23:24] = -InitWeights
        # WeightersCentre3[10:11, 46:47] = -InitWeights

        WeightersCentre3[11:13, 1:2] = InitWeights/2
        WeightersCentre3[11:13, 24:26] = -InitWeights/2
        # WeightersCentre3[11:13, 47:49] = -InitWeights

        WeightersCentre3[13:14, 7:8] = InitWeights/2
        WeightersCentre3[13:14, 26:27] = -InitWeights
        # WeightersCentre3[13:14, 49:50] = -InitWeights

        WeightersCentre3[14:16, 0:1] = InitWeights/2
        WeightersCentre3[14:16, 27:29] = -InitWeights/2
        # WeightersCentre3[14:16, 50:52] = -InitWeights

        WeightersCentre3[16:17, 6:7] = InitWeights/2
        WeightersCentre3[16:17, 29:30] = -InitWeights
        ...
        ADSA_leftarm.InitRecording()
        ADSA_rightarm.InitRecording()
        ADSA3.InitRecording()

        CPG = FN.FHNN(NumberOfNeurons=8, scale=0.02)
        CPG.InitRecording()        
    #%%
#        NeuronSensitivity = np.ones(num_actions*2)*0.005
        NeuronSensitivity = np.ones(8)*0.5
    ...
    for i_episode in range(N_episode): # run 20 episodes
        ...
        while True: # run until episode is done
            ...
            # observation = range_adapter.step_dynamics(dt, observation)
            ...
            # range_adapter.memory_maintenance()
            ...
            CPGr1Input = np.dot(ADSA_r1.Weighters, np.hstack((CPG.Vp[0], CPG.Vp[3], CPG.Vp[7], Neuron2Out,1)))
            ...
            CPGleftInput = np.dot(ADSA_leftarm.Weighters, np.hstack((CPG.Vp[0], Neuron2Out,1)))
            CPGrightInput = np.dot(ADSA_rightarm.Weighters, np.hstack((CPG.Vp[1], Neuron2Out,1)))

            CPGInput  = np.squeeze(np.array([CPGl1Input, CPGr1Input, CPGl2Input, CPGr2Input, CPGl3Input, CPGr3Input, CPGleftInput, CPGrightInput])) * NeuronSensitivity
            ...
            ADSA_leftarm.StepSynapseDynamics(dt, T, reward)
            ADSA_rightarm.StepSynapseDynamics(dt, T, reward)
            # ADSA.Recording()
            # ADSA2.Recording()
            # ADSA3.Recording()
            # print(ADSA.Amp[0,0])
